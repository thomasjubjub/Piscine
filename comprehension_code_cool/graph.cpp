...
void Ecosysteme::Sauvegarder_Fichier( std::string nom_fichier)
{
    ...
    fichier << m_Influences.size() << std::endl;

    for (int i=0; i<m_Influences.size()-1; i++)
            {
    fichier << vecInfluence[i]->m_e1 <<" "<< vecInfluence[i]->m_e2<< " " << m_Influences[i].m_coefficient << std::endl;
    }

    fichier <<vecInfluence[m_Influences.size()-1]->m_e1 <<" "<< vecInfluence[m_Influences.size()-1]->m_e2 << " " << m_Influences[m_Influences.size()-1].m_coefficient;
...
void Ecosysteme::update()
{
    ...
    for (auto &elt : m_Espece)
        ...
        if(key[KEY_S])
        {
             sauvegarde_coords("sauvegarde.txt");
             Sauvegarder_Fichier("Eco1.txt");
        }
