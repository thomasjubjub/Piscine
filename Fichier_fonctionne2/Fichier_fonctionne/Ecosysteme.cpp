...
void Ecosysteme:: Evolution_Temporelle()
{
    ...
    for(int i=0; i<vecEspece.size(); i++)
   {
       ///On calcule le nombre de successeurs
       successeurs =0;
        for (it = vecInfluence.begin(); it!=  vecInfluence.end(); it ++)
        {
            if((*it)->m_e2==i)
        ...
        cout << vecEspece [i]->m_nom << "  a : " <<successeurs<< "succ" << endl;
        cout << vecEspece[i]->m_N << endl;

        ///On calcule N en partant du haut
    if (successeurs ==0)
    {
...
{
    vector <Espece*> :: iterator it;
    string Espece_sup;
    cout << "Quel animal veux-tu supprimer:" << endl;
    cin >> Espece_sup ;
...
void Ecosysteme :: Ajouter_Espece ()
{
    vector <Espece*> :: iterator it;
    string Espece_add;
    cout << "Quel animal veux-tu ajouter:" << endl;
    cin >> Espece_add ;
